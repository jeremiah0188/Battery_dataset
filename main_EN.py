...
.mobile-nav-item-btn.active button {
...
if current_page == "login" and not st.session_state.is_admin:
    ...
    with col:
        with st.container(border=True):
            st.markdown(
                "<h2 style='font-size: 32px; font-weight: 900; color: #0F172A; margin-bottom: 8px; text-align: center;'>Welcome Back</h2>",
                unsafe_allow_html=True
            )
            st.markdown(
...
# ----------------- Sign Up -----------------
    ...
    with col:
        with st.container(border=True):
            st.markdown(
                "<h2 style='font-size: 32px; font-weight: 900; color: #0F172A; margin-bottom: 8px; text-align: center;'>Create Account</h2>",
                unsafe_allow_html=True
            )
            st.markdown(
            ...
            c3, c4 = st.columns(2)
            with c3:
                if st.button("Already have an account? Sign In", use_container_width=True, key="back_login"):
                    st.session_state.current_view = "login"
                    st.rerun()
            with c4:
...
# ----------------- Homepage -----------------
    ...
    hero_html = (
        ...
        '<div class="hero-desc">We deliver high-fidelity, peer-reviewed battery datasets and robust metadata integration tailored to global energy research.</div>'
        '</div>'
        ...
        '<div>'
        f'<div style="font-size:64px; font-weight:900; color:#172554; line-height:1;">{len(public_df)}+</div>'
    ...
    with st.expander("Do I need an account to download data?"):
        st.write("No. All curated datasets are open-access and do not require an account to browse or download.")
    with st.expander("How long does the moderation review take?"):
        st.write("Our admin team typically reviews submissions within 48–72 hours.")
...
# ----------------- Browse Datasets -----------------
    ...
    with result_col:
        ...
        if not filtered_df.empty:
            ...
            html_parts.append(
                '<div class="dataset-list-header" style="display:flex; background-color:#F8FAFC; padding:12px 18px; font-size:12px; font-weight:700; color:#475569; border-bottom:1px solid #E2E8F0; text-transform:uppercase; letter-spacing:0.4px;">'
            )
            html_parts.append(
