...
bg_gradients = {
    ...
    "Settings": "linear-gradient(135deg, #F8FAFC 0%, #EEF2FF 50%, #F5F3FF 100%)",
    "Notifications": "linear-gradient(135deg, #F8FAFC 0%, #ECFEFF 50%, #F0FDF4 100%)"
}
...
<style>
    ...
    /* 顶部导航搜索栏（仅导航栏） */
    div[data-testid="stTextInput"]:has(input[aria-label="Top Search"]) input {
    ...
    div[data-testid="stTextInput"]:has(input[aria-label="Top Search"]) input:focus {
        border-color: #4A6D5F !important;
        box-shadow: 0 0 0 3px rgba(74,109,95,0.12) !important;
    }

    /* 导航 icon 按钮（settings / notifications） */
    .nav-icon-btn button {
...
with st.container():
    ...
    # 调整比例：菜单更靠左，右侧留空间给搜索/图标/登录
    col_logo, col_menu, col_right = st.columns([1.8, 6.7, 4.0], vertical_alignment="center")
    ...
    with col_right:
        # 右侧：搜索框 + Settings + Notifications + 登录/登出
        if st.session_state.current_view not in ["login", "signup"]:
            r1, r2, r3, r4 = st.columns([5.0, 1.0, 1.0, 3.0], vertical_alignment="center")
            ...
            with r4:
                ...
                with a1:
                    if st.button("Search", key="nav_search_btn"):
                        st.session_state.search_kw = top_search
...
# ----------------- 页面 H：Settings -----------------
    ...
    st.markdown('<div class="section-header header-blue"><h2>⚙️ Settings</h2></div>', unsafe_allow_html=True)

    tab_acc, tab_pref, tab_dataset_pref, tab_notify_pref = st.tabs([
        "Account Settings", "Preferences", "Dataset Preferences", "Notification Preferences"
    ])

    with tab_acc:
        with st.container(border=True):
            st.markdown("### A. Account Settings")
            st.write("Manage your basic account information.")
            c1, c2 = st.columns(2)
            with c1:
                st.text_input("Display Name", value="Jian Wu" if st.session_state.is_admin else "")
                st.text_input("Email", value="jian.wu@utbm.fr" if st.session_state.is_admin else "")
            with c2:
                st.text_input("Password", type="password", placeholder="Enter new password")
                st.text_input("Role", value="Admin" if st.session_state.is_admin else "Contributor", disabled=True)
            if st.button("Save Account Settings", key="save_account_settings"):
                st.success("Account settings saved.")

    with tab_pref:
        with st.container(border=True):
            st.markdown("### B. Preferences")
            st.write("Customize the default behavior of the portal.")
            c1, c2 = st.columns(2)
            with c1:
                st.selectbox("Default page", ["Homepage", "Browse Datasets", "Contribute Data", "About", "Contact"])
                st.selectbox("Theme", ["Light", "Auto"])
            with c2:
                st.selectbox("Items per page", [10, 20, 50, 100], index=1)
                st.selectbox("Language", ["English", "中文"])
            if st.button("Save Preferences", key="save_preferences"):
                st.success("Preferences saved.")

    with tab_dataset_pref:
        with st.container(border=True):
            st.markdown("### C. Dataset Preferences")
            st.write("Set your preferred dataset filters for faster browsing.")
            c1, c2, c3 = st.columns(3)
            with c1:
                st.selectbox("Preferred domain", ["Energy", "Battery", "Healthcare", "Manufacturing", "Transportation", "Other"])
            with c2:
                st.selectbox("Preferred chemistry", ["NMC", "LFP", "NCA", "LCO", "LMO", "Solid-state"])
            with c3:
                st.selectbox("Default sort", ["Most Recent", "A-Z"])
            if st.button("Save Dataset Preferences", key="save_dataset_preferences"):
                st.success("Dataset preferences saved.")

    with tab_notify_pref:
        with st.container(border=True):
            st.markdown("### D. Notification Preferences")
            st.write("Control which updates you want to receive.")
            c1, c2 = st.columns(2)
            with c1:
                st.toggle("Email notifications", value=True)
                st.toggle("Submission status updates", value=True)
            with c2:
                st.toggle("New dataset alerts", value=True)
...
# ----------------- 页面 I：Notifications -----------------
    ...
    st.markdown('<div class="section-header header-amber"><h2>🔔 Notifications</h2></div>', unsafe_allow_html=True)

    # 顶部操作栏
    with st.container(border=True):
        a1, a2, a3 = st.columns([1.2, 1.2, 2.2])
        with a1:
            st.button("Mark all as read", use_container_width=True, key="mark_all_read")
        with a2:
            st.button("Clear read", use_container_width=True, key="clear_read")
        with a3:
            notif_filter = st.selectbox("Filter", ["All", "Unread", "System", "Dataset", "Review"], label_visibility="collapsed")

    # 示例通知（Inbox样式）
    notifications = [
        {
            "title": "✅ Your dataset submission has been approved",
            "message": "The dataset 'Oxford Battery Aging Set' has passed moderation and is now publicly visible.",
            "time": "2 hours ago",
            "status": "Unread",
            "type": "Review"
        },
        {
            ...
            "message": "Your recently submitted dataset is currently being reviewed by the moderation team.",
            "time": "1 day ago",
            "status": "Read",
            "type": "Review"
        },
        {
            ...
            "message": "A new EIS dataset was added under the Energy > Battery category.",
            "time": "2 days ago",
            "status": "Unread",
            "type": "Dataset"
        },
        {
            "title": "⚠️ Source link validation failed",
            "message": "One submitted source URL appears invalid. Please verify the repository link and resubmit.",
            "time": "3 days ago",
            "status": "Read",
            "type": "System"
        }
    ...
    if notif_filter != "All":
        if notif_filter == "Unread":
            filtered_notifications = [n for n in notifications if n["status"] == "Unread"]
        else:
            filtered_notifications = [n for n in notifications if n["type"] == notif_filter]

    if filtered_notifications:
        for idx, n in enumerate(filtered_notifications):
            with st.container(border=True):
                c1, c2 = st.columns([5, 1.3])
                with c1:
                    st.markdown(f"**{n['title']}**")
                    st.caption(n['message'])
                    st.caption(f"Type: {n['type']}  |  Time: {n['time']}")
                with c2:
                    badge_color = "#166534" if n["status"] == "Unread" else "#64748B"
                    st.markdown(
                        f"<div style='text-align:right; font-weight:700; color:{badge_color};'>{n['status']}</div>",
                        unsafe_allow_html=True
                    )
                    st.button("Open", key=f"open_notification_{idx}")
    else:
        st.info("No notifications under this filter.")

# ----------------- 页面 J：Admin Dashboard -----------------
